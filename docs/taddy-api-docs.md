...
`REGULAR` - Podcast feed is checked every day. This is the most common queue for podcasts that do not support WebSub.
...
- We use queues to prioritize which episodes we transcribe. Newly released episodes are high priority and transcribed as soon as possible. We're also working through the entire back catalogue of the most popular podcasts, which is a lower priority.
...
2. You can use `transcriptUrls`or `transcriptUrlsWithDetails` to get the URLs where you can download a transcript for an episode. This includes both the transcripts provided by podcast (if available) and the transcripts that are automatically generated via our API. 
...
<aside>
👋 You get to try our transcription features for free. 

Every Taddy API user (Free or Paid) gets access to any episode transcript where the podcast provides their own transcript. ”Build Your SaaS” (uuid: “6bdfd429-f58b-427d-8072-353d478aa15f”) is an example of a podcast that provides transcripts for its episodes.

Being a Taddy API Business user allows you to transcribe any episode provided through our API as we generate transcripts when they are not provided.
...
To get the most popular podcasts in the Business genre. **Note:** filterByGenres takes an array of genres so that you can use multiple genres and subgenres. Please see [Genre](https://taddy.org/developers/podcast-api/genre) for a full list of possible genres.
...
The Listen Notes API and Taddy’s API both have the goal to help make it easy for you to build a great podcast app. Both APIs do the backend tasks involved with keeping podcast and episode data up-to-date (by constantly crawling RSS feeds to check for updates). There are 2 key differences between the Listen Notes and Taddy APIs:  

1) The control you have over the data you get back from each API. The Listen Notes API [Terms of Service](https://www.listennotes.com/api/terms/) does not allow you to save the information you get back from its API on your server, this same restriction does not apply when using Taddy’s API. 

2) You are required to promote Listen Notes in your app via a [“Powered by Listen Notes”](https://www.listennotes.com/api/terms/) banner, this same restriction does not apply when using Taddy’s API. 

Below we go over a feature by feature comparison to help you get a summary of what working with each API would be like and why we concluded **Taddy’s API is the winner** and why it is a great alternative to the Listen Notes API.
...
| Webhooks | ✅ - Get a [webhook](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) notification whenever a new podcast or episode is released. | ❌ |
| Save data from API on your server | ✅ | ❌ |
| White-label solution | ✅ | ❌ (”Powered by Listen Notes” Logo required) |
| Type of API | GraphQL | REST |
| Price | Free -  500 API Calls per month.

[Paid - $50/month](https://taddy.org/developers/pricing) for up to 100,000 API Calls per month | Free -  300 API Calls per month.

[Paid - Starts at $180/month](https://www.listennotes.com/api/pricing/) for up to 5,000 API Calls per month |

### Getting details on a podcast

Both APIs give you way to get back details on a podcast.
...
Both APIs give you way to get back details on an episode.
...
Both APIs give you way to perform a text search on millions of podcasts & episodes.
...
Taddy API uses a GraphQL API. It allows you to construct a query and only get back the exact data you care about.

Listen Notes API uses a REST API. REST will be more familiar to most developer.  

Winner: **Tie** (This will be very depended on what API frameworks a specific developer is familiar with. Most developer will be more familiar with REST but GraphQL is easy to setup and gives a lot more control to the developer using the API)

### Ability to save data returned by the API

Taddy API allows you to save returned from Taddy’s API and use . Taddy also  

As per the Listen Notes API [Terms of Service](https://www.listennotes.com/api/terms/) you are not allowed to save any data retuned via the Listen Notes API on your server.
...
As per the Listen Notes API [Terms of Service](https://www.listennotes.com/api/terms/) you are required to add a “Powered by Listen Notes” badge to your user interface.
...
Listen Notes API: 2+ million podcasts, 130+ million episodes (Caveat: The difference in total episodes is mainly due to episodes that have been removed from a podcast’s RSS feed but are still showing up in the Listen Notes API)

Winner: **Taddy** 

### Hosted Podcast Artwork

Listen Notes API uses the image from the RSS feed and hosts it on their own image servers. Using a consistent image server gives you better speed and the ability to easily resize the images.

Taddy API does not rehost images, but iTunes does, and we link to the iTunes image link within our API. The drawback of this is if a podcast is not on Apple Podcasts (iTunes), you will have to fallback to using the image from the RSS feed itself.

Winner: **Listen Notes**

### Links to 3rd party websites

Only Listen Notes API keeps track of links to 3rd party websites like Twitter, Instagram, Facebook, YouTube, Spotify for the podcast.

Winner: **Listen Notes**

### Webhooks

Only Taddy API gives you ability to setup a [webhook](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) endpoint where you will receive notifications whenever there is a new or updated podcast or episode. This can be a critical feature depending on what podcast app you are trying to build because it means you don't have to constantly check Taddy’s API to see if a podcast has updated information, Taddy will push these updates immediately to you. For example, a common use case is you can immediately send a push notification to your users about a newly released episode. 
...
Free Plan - Up to 500 API Calls / month

Pro Plan - $50/month for up to 100,000 API Calls

Listen Notes API: 

Free Plan - Up to 300 API Calls / month

Pro Plan: 

$180/month for up to 5,000 API Calls. 

$332/month for up to 100,000 API Calls

Winner: **Taddy**

## Summary

Both the Listen Notes API and Taddy API are well documented and well built APIs. The two main drawbacks with the Listen Notes API is the price and the control you get over the data you get back from the API and that is why we concluded **Taddy’s API is the winner** and a great alternative to the Listen Notes API. Also, make sure to check our list of the [🥇Top 5 Podcast APIs](https://taddy.org/blog/best-podcast-api-tools) if you would like a summary of all the different Podcast API options available.
...
`PROCESSING` - Currently in a queue, waiting to be transcribed.

`COMPLETED` - Transcript and chapter details are now available.
...
PodcastIndex’s Podcast API and Taddy’s API both have the goal to help make it easier for you to build a great podcast app. Both APIs do the backend tasks involved with keeping podcast and episode data up to date (like constantly crawling RSS feeds to check for updates) and serve up-to-date information via their API.  

Below we go over a feature by feature comparison to help you get a summary of what working with each API would be like and why we concluded **Taddy’s API is the winner** and a great alternative to the PodcastIndex’s Podcast API.
...
[Paid - $50/month](https://taddy.org/developers/pricing) for up to 100,000 API Calls per month | 100% Free |
...
PodcastIndex API: See [**/podcasts/id**](https://www.listennotes.com/api/docs/#get-api-v2-podcasts-id) endpoint.
...
PodcastIndex API: See [**/episodes/id**](https://www.listennotes.com/api/docs/#get-api-v2-episodes-id) endpoint.
...
PodcastIndex API: See [**/search**](https://www.listennotes.com/api/docs/#get-api-v2-search) endpoint.
...
PodcastIndex’s API uses a REST API. REST will be more familiar to most developer.  

Winner: **Tie** (This will be very dependant on what API frameworks a specific developer is familiar with. Most developer will be more familiar with REST but GraphQL is easy to setup and gives a lot of control to the developer using the API)

### Total Number of Podcasts & Episodes available

Taddy API: 4+ million podcasts, 95+ million episodes

PodcastIndex API: 4+ million podcasts, does not specific how many episodes

Winner: **Tie**

### Webhooks

Only Taddy API gives you ability to setup a [webhook](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) endpoint where you receive notification whenever there is a new or update podcast or episode. This can be a critical feature depending on what you are trying to build because it means you don't have to constantly make API requests to Taddy to check if a podcast has updated information or there are newly released episodes, Taddy will push the updates immediately to you. You can then act on that information ex) send a push notification to all your users that there is a newly released episode. You will need a paid Taddy account setup a webhook.
...
Both the PodcastIndex API and Taddy API are great APIs to use if you are building a podcast app. If you don’t need the extra features in Taddy’s API and a free API is a major requirement for you, PodcastIndex’s API is exactly what you need. If you are building a podcast app that needs search and webhooks, Taddy’s API a great alternative to the PodcastIndex API. Also, make sure to check our list of the [🥇Top 5 Podcast APIs](https://taddy.org/blog/best-podcast-api-tools) if you would like a summary of all the different Podcast API options available.
...
You can filter your search for results from a specific country, genre, language, from a particular podcast, before or after a certain publish date, before or after the latest episode was published, or if there is a transcript available. You can see the full list of filtering options available in the [Query Breakdown](https://www.notion.so/Search-for-podcasts-or-episodes-17524604b115814caed6f1429f20887f?pvs=21) section below. 
...
**Note:** All Taddy users can view transcripts that have been provided by the podcast, but you will need to be a Taddy Business user to view [transcripts that we automatically generate](https://www.notion.so/Get-Episode-Transcripts-17524604b11581618985e7a1bd869465?pvs=21).  
...
The response you get back includes an array of [PodcastSeries](https://www.notion.so/PodcastSeries-17524604b11581f88332d11ae58d5103?pvs=21) and [PodcastEpisodes](https://www.notion.so/PodcastEpisode-17524604b11581b19460cbf4934e5169?pvs=21) that match your search term.
...
**TLDR:** The best in class API for podcasts. It saves you from a doing any of the backend work needed to constantly check podcast feeds. It has a free tier but if you want all the features (like webhooks notifications) you will need the paid version.
...
✅ [Webhook notifications](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) for newly released podcasts or episodes.
...
**TLDR:** Authentication was hard to setup with this API, but once you have it setup, it is one of the largest directories of podcasts and gives you information on all podcasts and episodes. It is also 100% free.
...
**TLDR:** Listen Notes has great documentation and is a well built API. But, there are two main drawbacks with the Listen Notes API: 

1) It is quite expensive and 

2) As per their [terms of service](https://www.listennotes.com/api/terms/), you are not allowed to save any information you get from their API on your servers, which will lead to you making a lot more API requests (and paying more for those requests). 
...
❌ Requires to show a [“Powered by Listen Notes”](https://www.listennotes.com/api/terms/) Logo in your application

❌ You are [not allowed](https://www.listennotes.com/api/terms/) to save any information you get back from the Listen Notes API on your servers. 
...
**TLDR:** Spotify is one of the fastest growing podcast players and therefore a lot of podcasts add their podcast to Spotify’s Podcast Directory and are therefore accessible through their Spotify  API. However, the main drawback is to be aware of the Spotify API Rate Limits or require every user that uses your app to Login with their Spotify account (so every user has their own Rate Limit). 
...
**TLDR:** Apple Podcasts is the largest podcast player on and therefore most podcast creators want to be listed on Apple Podcasts and therefore the iTunes API. The iTunes API is a great place to get a list of all the podcasts available (ie: a directory) but it doesn't try to more than that. 
...
[**📻 Taddy Podcast API →**](https://taddy.org/developers/podcast-api)
...
- For Episodes, you can get Top Charts by country.
...
- Are you **building a podcast app?**
    - Do you want to make sure your app is always up to date with the latest podcasts? Get a webhook notification immediately when a new podcast is added to our directory.
    - Do you want to push notify your users whenever a new episode is released? Get a webhook notification immediately when a new episode is released. On your server, you  match the users that want a push notification for new episodes and send one to them.
- Do you care about **SEO?** Get a webhook notification immediately whenever your website gets linked to on a new episode.
- Do you provide **transcription** or **translation** or **audiogram** creation services? Get a webhook notification immediately after your customer releases a new episode. Remind them to use your product (or proactively reach out to them if haven't been using your service)
...
The key difference between the iTunes and Taddy APIs is that they are both trying to accomplish different goals. The iTunes API is a **directory** **of all the podcasts** that are on Apple Podcasts, while Taddy API’s **makes it easy for you to build a podcast app**. Taddy’s API does that by doing the hard backend tasks involved with keeping podcast and episode data up to date (like constantly crawling RSS feeds to check for updates). iTunes API does not do this, but gives you the urls for all the podcast feeds so that you can do that yourself.

So, which API you should use will depend on your specific business use-case. Below we go over an example of getting back data from each API and a feature by feature comparison to help you get a summary of what working with each API would be like.
...
The most useful property you get of all of these is the `feedUrl`. It is the link to the podcast’s RSS feed where you can get more details on the podcast and all episodes. **But that is also the main difference between the iTunes API and Taddy’s API**, you would then have to go to the url for each podcast to get the current details and constantly check it for updates (like new episodes released). 

That is why the iTunes API is more a directory of all the podcasts vs an API for you to build your next podcast app on. However, if all you need is a list of all RSS feeds for all the podcasts on Apple Podcasts, this API is exactly what you need.

## How to use Taddy’s API to get Podcast details

The goal of Taddy’s API is to help you build a great podcast app. The API lets you to do all the following:
...
- Get a [notification](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) whenever there is new or updated podcast
- Get a [notification](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) whenever there is new or updated iTunes information for a podcast
- Get a [notification](https://www.notion.so/5a0379efd25d42f2a2dcd35245bbb5bf?pvs=21) whenever there is new or updated episode
...
Notice that this time you get back more information about the podcast. Specifically you get information from the podcast’s RSS feed, like `name`, `description`, `imageUrl` and `episodes`. Taddy API is constantly checking feeds for updates, so you also be sure that the information is up-to-date without you having to constantly do those checks yourself. If that is something you need, then using the Taddy API is a great alternative to using the iTunes API.
...
[Paid - $50/month](https://taddy.org/developers/pricing) for 100,000 API Calls per month | 100% Free |

The reason “Get details on a podcast” is given ½ is because the iTunes API just gives you the url of a RSS feed for the podcast, it doesn't give you any other podcast details, you have to go to the url to get those details yourself.

## Summary

In summary, which API you use will depend on your specific business use-case. If all you need is a directory of all the podcasts, iTunes API is 100% free and all you need, if you need some more features to help you build out your podcast app, you should check out Taddy’s API. Also, make sure to check our list of the [🥇Top 5 Podcast APIs](https://taddy.org/blog/best-podcast-api-tools) if you would like a summary of all the different Podcast API options available.
...
GraphQL Type for additional information from iTunes on a particular podcast. (A small number of podcasts are not on iTunes and those will not return any iTunesInfo details)
...
- [x]  [Episode Transcripts](https://taddy.org/developers/podcast-api/episode-transcripts) and [Chapters](https://taddy.org/developers/podcast-api/episode-chapters) for the most popular podcasts. 1000 credits per month.

- [x]  On-demand [transcripts](https://taddy.org/developers/podcast-api/episode-transcripts##a5858f5ae4544bc69590c42a3d95bc83) for any episode. 150 credits per month.
...
$50/month (Pro) - 100k API requests per month

$100/month (Business) - 350k API requests per month. 1000 Transcription credits (most popular podcasts). 150 on-demand transcription credits. 

$200/month (Business) - 1 Million API requests per month. 1000 Transcription credits (most popular podcasts). 150 on-demand transcription credits. 

Additional Regular transcription packs - 2500 credits for $100/month (4c per transcript)

Additional On-Demand transcription packs - 500 credits for $100/month  (20c per transcript)
...
[Country](https://www.notion.so/Country-7fde6d1a3f3e4da3beb97f2b64ee6b03?pvs=21)
...
- Are you **building a podcast app?**
    - Do you want to make sure your app is always up to date with the latest podcasts? Get a webhook notification immediately when a new podcast is added to our directory.
    - Do you want to push notify your users whenever a new episode is released? Get a webhook notification immediately when a new episode is released. On your server, you can match the users that want a push notification for new episodes and send one to them.
- Do you care about **SEO?** Get a webhook notification immediately whenever your website gets linked to on a new episode.
- Do you provide **transcription** or **translation** or **audiogram** creation services? Get a webhook notification immediately after your customer releases a new episode. Remind them to use your product (or proactively reach out to them if haven't been using your service)
